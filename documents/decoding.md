...
func main() {
        raw_json := []byte (`{"files": [{"filename": "style.css","filedata": "body {color:black}","directory": "css"},{"filename": "main.js","filedata": "console.log('yo')","directory": "js/sub/"},{"filename": "index.html","filedata": "<html><body>this is it</body></html>","directory": ""}]}`)

        // Map of interfaces can receive any value types
        u := map[string]interface{}{}
        err := json.Unmarshal(raw_json, &u)
        if nil != err {
                panic(err)
        }

	files := u["files"].([]interface {})
	first := files[0].(map[string]interface {})
	fmt.Printf("First: %s\n", first )
	fmt.Printf("First FileName: %s\n", first["filename"].(string))
}
```
