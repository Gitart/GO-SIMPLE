...
A fairly close port of the Common Lisp solution, this solution uses the go.matrix library     
for supporting functions. Note though, that go.matrix has QR decomposition, as shown in the Go solution   
to Polynomial regression. The solution there is coded more directly than by following the CL example here.    
Similarly, examination of the go.matrix QR source shows that it computes the decomposition more directly.  
