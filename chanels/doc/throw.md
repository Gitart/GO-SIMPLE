...
Go популяризировал мантру , не обмениваясь памятью; обмениваться памятью путем общения. У языка есть традиционный мьютекс (конструкция взаимного исключения) для координации доступа к общей памяти, но она способствует использованию каналов для обмена информацией между горутинами.

В этой статье короткий взгляд на goroutines, нити и условия гонки показывают пример для просмотра двух программ Go. В первой программе goroutines взаимодействуют через синхронизированную общую память, а вторая использует каналы для той же цели.
...
func updateBalance(amt int32) {        
      atomic.AddInt32(&accountBalance, amt) // no explicit locking required  не требуется явная блокировка 
}
```
